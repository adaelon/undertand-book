...
- **来源三分(重划)**:① 确定性账本(无 LLM,直存)= 真读过的 LID 历史 + `note/highlight/qa`(用户显式动作);② 用户主动 LLM 记忆(LLM,**用户信号触发、非后台全量扫描**)= 用户显式「记下 X」/ 跨轮反复提及,**认知诚实标注「用户多次提到 X」(非「对 X 有兴趣」)+ citation 锚真 LID**;③ 会话临时(`dialogue/position`,不持久)。
