...
## 段 (paragraph / 叶子块)
切分的中间叶子单元 = 源格式的一个块级标记(md 段/列表项/引用块/代码块;epub p/li/blockquote/br)。**忠实映射源块的结构类型,不检测文学体裁**;诗行、对话轮靠块边界天然落位(非按体裁识别),裸 txt 退化到空行。切片0 的最深 LID 层。注:`code/table/image` 三类块级标记是带类型的一等 **asset 叶子**(见下),非普通 paragraph。状态:NEW(详见 [docs/adr/0008/0029])。

## asset 叶子 (asset leaf)
代码块 / 表 / 图三类**带类型的一等 LID 叶子** `[ADR-0029]`(`NodeKind ∈ {Code,Table,Image}`,闭集可扩展)。仍是 LID 树叶子:**占 source span、进分区不变式划分、用 LID 单一寻址**(不引第二套 asset_id)。**原文 = 源标记的确定性序列化**(image=`![alt](src)`、code 保留换行缩进、table 保留表文本),`book.text(asset_lid)` 返回它,确定性+忠实;`book.manifest` 节点带 `kind` 暴露可见性(零新命令,agent 过滤 kind 定位)。图谱层与 paragraph **一视同仁**(普通可锚 LID,确定性图谱闸不改)。多模态图理解(描述图)留切片1+、走"标注来源"旁路、**不进只读基座**。源自 `参考.md` 文档世界状态「让图/表/代码不从链路消失」。状态:NEW(详见 [docs/adr/0029])。
...
## 记忆 consolidation(两阶段后台 + 来源三分 + 四层渐进披露 + 遗忘分裂)
memory 层的后台 LLM 流水线,参照 Codex `codex-rs/memories` 落地 `[ADR-0018]`(命令面记录模型见 [docs/adr/0015])。
- **来源三分**:① 显式 save = 确定性用户产物(`note/highlight/qa`,reader 委托 memory.save,LLM 不抽);② Phase1 抽取 = 跨轮涌现推断(`interest/sticking_point/journey`,带证据→推断认知诚实标注 + 最小信号闸 no-op);③ 会话临时(`dialogue/position`,Phase1 只消费)。
- **Phase1(per 阅读片段)**:抽取单元 = 自上次以来未抽交互流;**idle 切片段 + 多触发取先到**(idle 超时主 / 关书切书退出即时 / 超量超时保护),后台扫 idle 够久者抽,绑 `source_session_id`。改 Codex 的 per-rollout(阅读是跨多次打开的连续流)。
- **Phase2(global)**:单全局锁 + INIT/INCREMENTAL 双模式 + git baseline diff 驱动,产**四层渐进披露产物**:`reader-profile`(常驻系统提示,类 memory_summary)/ `阅读手册`(可 grep,**per-book + cross-book 双维聚类**,类 MEMORY.md)/ `session 详档`(类 rollout_summaries)/ `raw 原料`(临时,类 raw_memories)。**skills 不由 consolidation 产**(= [docs/adr/0005] §5.4 预设工具集)。
- **遗忘按来源分裂**:确定性用户产物**永不 usage 遗忘**、只用户显式 `memory.delete`(笔记是事实);Phase1 推断走 **usage 剪枝 + 时效遗忘 + 矛盾遗忘**(推断会过时)。改 Codex 的"输入被删驱动遗忘"(我们阅读历史不可删)。
状态:NEW(详见 [docs/adr/0018])。
