...
Revised by: ADR-0116；`2,048 estimated tokens / 8,192 bytes` 是当前本地保守 profile，不是已证明的 Codex/MCP 宿主硬上限。ADR-0117；丢失的 `generation.start` 只在当前 active 恢复代际内重放，stale/expired session 必须换代。
Change type: [边界重构]。
