...
- **Web reader** owns ephemeral interaction state and calls localhost Reader APIs. Its Markdown body follows the restored `e64d5f0` path: App hydrates each `visible_lid`, replaces or appends/prepends unique segments, and `ReaderPane` projects the continuous flow while requesting the next interval at an edge. PHR bounded eviction, range hydration, stable-segment caching, and the uncommitted native-prose variants are not active. ([ADR-0112](adr/0112-pre-phr-reader-body-path-rollback.md)) `usePdfSelectionDraft` owns native selection resolution; the independent `usePdfSelectionTranslation` controller owns only translation request state.
- **server::host** owns sockets, worker threads, the global `AppState` mutex, Provider configuration snapshots, and lock boundaries. Its read-ledger worker waits for a 250 ms navigation idle period, then locks the same `AppState`/`MemoryStore` to coalesce pending LID touches into one atomic commit; book switch and orderly shutdown force a final attempt. ([ADR-0106](adr/0106-asynchronous-coalesced-read-ledger-persistence.md))
...
The restored Markdown reader body uses the pre-PHR interval path:

```text
reader.state / reader.goto / reader.scroll -> Viewport.visible_lids
  -> api.text(lid) for each visible leaf + singular FormulaSemantics lookup when needed
  -> Segment[] in viewport order
  -> replace for navigation | append/prepend unique LIDs for edge loading
  -> ReaderPane continuous flow
  -> scroll probe emits current LID and edge proximity requests the next Reader interval
```

Read-ledger durability remains outside the navigation response:

```text
POST reader.goto | reader.scroll
  -> lock AppState -> update interval viewport
  -> enqueue visible {book_id, lid, touch_count, last_seen} in MemoryStore
  -> return viewport without MemoryStore disk I/O
  -> 250 ms navigation idle
  -> Host worker locks the same AppState/MemoryStore
  -> merge pending LIDs into one MemoryDocument candidate + atomic commit
...
- **Asynchronous coalesced read ledger**: [ADR-0106](adr/0106-asynchronous-coalesced-read-ledger-persistence.md).
- **Pre-PHR reader body path rollback**: [ADR-0112](adr/0112-pre-phr-reader-body-path-rollback.md).
