...
> 现行契约 = `需求文档-V3.md`;术语 = `CONTEXT.md`;决策 = `docs/adr/0001`–`0020`、`0044`、`0045`。
...
flowchart TB
  subgraph BUILD["① 预构建期(绑 agent harness,一次性、离线)"]
    ...
    CAT --> P2[可选 Pass2 长程边<br/>全量目录+5 并发]
    CAT --> BS[BookStructure sidecar<br/>unit cards + stitching]
    P2 -. accepted/pending 边增强 .-> BS
    BS --> GATE[分区不变式/锚定率自检闸]
  ...
  subgraph READ["② 读时(与 harness 脱钩,独立产品)"]
    ...
    subgraph CMD["命令面 · 三命名空间(人机同面,无特供)"]
      BOOK["book.* 只读<br/>manifest/context/text/concept/structure/guide_path(叶子工具)<br/>query/synthesize(LLM 运行时暴露)"]
...
纯流水线,**确定性骨架 + LLM 只管语义**(借 U-A 纪律,见 memory `understand-anything-reference`)。骨架固定,抽取规则由 content profile / extraction rule pack 插槽驱动:`technical_learning` 是当前默认规则包,`paper` 是新增规则包 `[ADR-0048]`。

| 组件 | 定位 | ADR |
| --- | --- | --- |
| content profile / extraction rule pack | 挂到 Pass1 / profile-sidecar / Pass2 / BookStructure / MCP projection 的可插拔规则包;不得改 LID/source/book.text/citation/gate | 0033/0048 |
...
| `pass2-longrange-linker` | 用户在 BuildPlan 中选择的可选增强;启用时以全量目录 + 窗口正文抽长程边、不产节点,且与 Pass1 保持硬串行屏障 | 0010/0098 |
| PaperMetadata sidecar | `paper` profile 独立物化 `paper_metadata.json`;字段统一 `MetadataField<T>` 来源信封,不塞 BookStructure/graph | 0050/0051/0052 |
| PaperLexicon sidecar | `paper` profile 独立物化 `paper_lexicon.json`;只存论文术语索引和 LID occurrences,不存普通词典或长中文解释 | 0054/0055/0056/0057 |
| BookStructure sidecar | 从公共基座/profile artifacts 预构建 `spine + throughlines + key_stops` 结构地图;Pass2 audit 存在时注入长程边,不存在时使用空长程边 | 0044/0098 |
...
| 命名空间 | 可变性 | 内容 | ADR |
| --- | --- | --- | --- |
| `book.*` | 只读 | `manifest/context/text/concept` = 确定性**叶子工具**;`structure/guide_path` = BookStructure 读时投影;`query/synthesize` = LLM 运行时暴露 | 0014/0016/0017/0045 |
...
源文件 → LID 切分(段/句,确定性) → 窗口切分(LID 子树+预算)
  ...
  ├─ 用户启用 Pass2 → 抽长程边(全量目录,5并发) → merge+闸 ─┐
  └─ 用户禁用 Pass2 ───────────────────────────────────────┤
                                                           → BookStructure sidecar(unit cards + stitching) → 最终图
  → 分区不变式+锚定率自检闸 → 固化只读基座
```
...
用户问 → E 外层 loop:LLM 自主编排
  ...
  ├─ 主动带读先调 book.structure(anchor)+book.guide_path(anchor)拿结构位置与宏观路线
...
| BookStructure 读时投影 | 可选 sidecar 缺失显式 unavailable,坏 sidecar fail-fast;结构路线只读投给人/agent/MCP | 0044/0045 |
...
**读时 Rust crate 依赖拓扑**(无环 DAG `[ADR-0027]`):`base-schema`(权威 schema)→ `read-tools`(确定性叶子工具 `Book`,读入可选 `book_structure.json` 并投影 `structure/guide_path`)→ `memory`(用户私有记忆层,物理隔离 `[ADR-0006]`)→ `reader`(命令优先阅读器,委托 memory)→ `runtime`(模块 E 双层 loop + ModelAdapter,顶层编排 book.*/reader.*/memory.*)→ `server`(localhost REST/MCP 投影,tiny_http 同步,持 `Arc<Mutex<AppState>>` 单会话 `[ADR-0028]`;S10a 仅依赖 read-tools,S10b+ 扩 reader/memory/runtime)。箭头 = 被依赖方向;`memory`/`reader` 各自独立 crate 是为拆 `runtime↔reader` 循环依赖(crate 强制 DAG)+ 兑现记忆层独立地位 `[ADR-0027]`。`server` 坐 DAG 顶端,把冻结命令面(V3 §4)投影成 HTTP/MCP 给「人/前端/访客」,与库内 agent 共用同一命令面(人机对称 `[ADR-0007]`)。
