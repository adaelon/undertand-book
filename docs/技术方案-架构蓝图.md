...
**读时 Rust crate 依赖拓扑**(无环 DAG `[ADR-0027]`):`base-schema`(权威 schema)→ `read-tools`(确定性叶子工具 `Book`)→ `memory`(用户私有记忆层,物理隔离 `[ADR-0006]`)→ `reader`(命令优先阅读器,委托 memory)→ `runtime`(模块 E 双层 loop + ModelAdapter,顶层编排 book.*/reader.*/memory.*)→ `server`(localhost REST 投影,tiny_http 同步,持 `Arc<Mutex<AppState>>` 单会话 `[ADR-0028]`;S10a 仅依赖 read-tools,S10b+ 扩 reader/memory/runtime)。箭头 = 被依赖方向;`memory`/`reader` 各自独立 crate 是为拆 `runtime↔reader` 循环依赖(crate 强制 DAG)+ 兑现记忆层独立地位 `[ADR-0027]`。`server` 坐 DAG 顶端,把冻结命令面(V3 §4)投影成 HTTP 给「人/前端」,与库内 agent 共用同一命令面(人机对称 `[ADR-0007]`)。
