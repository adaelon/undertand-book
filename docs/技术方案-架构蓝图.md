...
**打包形态** `[ADR-0022]`:整体 = **本地 Claude Code 插件外壳**(U-A 同构,「官方」非必需)。`.claude-plugin/plugin.json` + `skills/understand-book/SKILL.md`(一句话入口 `/understand-book <epub|md>`)+ `agents/{pass1,pass2}.md`(harness 供 LLM)编排预构建管线;读时启动亦走插件 skill(类 U-A `/understand-dashboard`)拉起 Rust localhost 服务,服务本体与 harness 脱钩。插件面(`.claude-plugin`/`skills`/`agents`)与 `packages/`(TS)、`crates/`(Rust)并置于同一 monorepo。
