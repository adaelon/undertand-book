...
flowchart TB
  subgraph BUILD["① 预构建期(绑 agent harness,一次性、离线)"]
    ...
    P2 --> M2[merge+确定性闸] --> BS[BookStructure sidecar<br/>unit cards + stitching]
...
| BookStructure sidecar | 从公共基座/profile artifacts 预构建 `spine + throughlines + key_stops` 结构地图,供带读先总览再停靠 | 0044 |
...
源文件 → LID 切分(段/句,确定性) → 窗口切分(LID 子树+预算)
  ...
  ──硬串行屏障──
  → Pass2 抽长程边(全量目录,5并发) → merge+闸 → BookStructure sidecar(unit cards + stitching) → 最终图
  → 分区不变式+锚定率自检闸 → 固化只读基座
```
