...
MemoryStore `[ADR-0075]`
  ...
  -> reading_state.rs: BookReadingState raw engagement reducer + legacy projection
...
Runtime review execution `[ADR-0075]`
  -> memory_review.rs: immutable ReviewInput/ReviewTurnInput + ReviewExecutor/Factory seam
...
Durable profile review state `[ADR-0075]`
  ...
  -> pure job/error/watermark changes increment document_revision only;
     fact-producing review commits will also advance projection_revision in M2.5
...
Independent review execution `[ADR-0075]`
  ...
  -> short AppState lock after execution
      ...
      -> successful raw output persists Retryable + REVIEW_RESULT_PENDING_VALIDATION
         and returns the immutable output to the caller
  -> M2.4 deliberately has no extraction prompt/validator/fact commit;
     M2.5 replaces the success placeholder with atomic facts/observations/watermark completion
```
...
Reader profile snapshot projection `[ADR-0075]`
  ...
  -> MemoryStore::project_reader_profile_snapshot
      ...
      -> exclude unvalidated Extension payloads until M3 policy schema validation exists
  ...
  -> POST /agent/chat
      -> build SnapshotRequest from current book/profile/time before resident run
...
Book reading state `[ADR-0075]`
  -> MemoryStore::derive_book_reading_state(book_id)
      ...
      -> qa_count,note_count,highlight_count,last_seen_at remain raw behavior facts
