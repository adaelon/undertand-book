...
Resident BuildIntent control plane `[ADR-0093]`
  ...
  -> POST /build_intent/draft
      ...
      -> Server resolves listed/private/one-off selections through stdin-only intent.blueprint
      -> stdin-only intent.plan invokes the deterministic V2 Core compiler and freezes full Blueprint snapshots
...
Reader-private Blueprint-driven artifact acceptance `[ADR-0094]`
  -> confirmed goal-directed BuildIntentAny + BuildPlanAny
      ...
      -> root handoff receives only task/artifact compatibility identity and an opaque private task path
