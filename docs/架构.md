...
Build capability registry and deterministic compiler `[ADR-0093]`
  ...
  -> read_now emits no plan;standard_deep emits only public closure;goal_directed emits only declared private artifacts and dependencies
...
Runtime Agent request governance `[ADR-0091]`
  ...
  -> tool_exposure.rs: profile/permission/evidence/model-budget direct/deferred/hidden projection
      -> ordinary sampling exposes at most 8 direct schemas,including metadata-only tool.search
  ...
  -> agent_prompt.rs: base identity + evidence/source/discovery/navigation/Reader/paper/memory/profile/finish policies
      -> only modules supported by the current sampled tool surface enter that request
...
```text
Capability registry and BuildPlan compiler `[ADR-0093]`
  -> structured mode + BuildIntentV1 -> exact registry selection;unknown/custom-auto selection fails closed
  -> public.standard_deep profile entry stages -> current BUILD_STAGE_DAG -> stable semantic closure
...
```text
Confirmed BuildPlan execution gate `[ADR-0093]`
  ...
  -> standard_deep executes the exact profile DAG closure;goal_directed executes only a stable dependency-closed subset
...
Codex plugin one-command prebuild
  ...
  -> nextAutomaticBuildAction selects only the first unfinished stage
      -> extract: run deterministic input command
          ...
          -> a fourth semantic attempt returns retry_exhausted;lease epoch overflow returns executor_instability
          ...
          -> semantic failure diagnostics retry up to three consecutive semantic attempts;reset clears the counter without reusing physical attempt numbers
...
Resident BuildIntent control plane `[ADR-0093]`
  ...
  -> POST /build_intent/draft
      ...
      -> goal_directed sends raw goal only to Runtime structured completion
          -> strict registry/schema/current-book LID gate returns a candidate without raw goal
      -> stdin-only intent.plan sidecar invokes the deterministic Core compiler
      -> draft intent/plan bodies live under the reader-private root
  -> POST /build_intent/edit
      -> no Provider call;existing structured candidate or explicit edited candidate
      -> the same Runtime catalog gate rejects unknown capability and out-of-book scope
...
```text
Reader-private artifact acceptance `[ADR-0093]`
  -> confirmed goal-directed BuildIntent + BuildPlan
      -> Core recomputes intent/plan identity and current source/profile binding
      -> current book LID catalog resolves the confirmed source scope
      -> each private artifact freezes one task envelope
          -> raw goal and allowed LIDs stay inside the private envelope
          -> output schema/rules come from the existing Sidecar contract registry
          -> root handoff receives only task identity and an opaque private path
  -> task-owned candidate returns to the deterministic Core gate
      -> artifact owner digest and independent attempt directories prevent cross-task relay
      -> executor reads task.json and writes only the sibling candidate.json
      -> failure detail stays private;retry opens a new attempt without a shared ledger
      -> strict candidate shape rejects path/relay metadata
      -> current task is recompiled before acceptance, blocking stale/tampered work
      -> every record binds at least one real, in-scope LID
      -> graph references and per-view invariants close before commit
  -> accepted private body and bounded receipt separate
      -> immutable accepted.json remains inside the reader-private artifact directory
      -> receipt contains only identity digests, counts, state, attempt, and timestamp
      -> stored receipts are revalidated against candidate and accepted digests before projection
  -> Server owns the resident-only active-overlay projection
      -> confirmation atomically selects the current intent/plan before artifact execution
      -> each declared artifact projects pending until its accepted body passes Rust identity/digest checks
      -> replan never falls back to an older goal overlay;accepted siblings remain independently readable
      -> Visitor and Book MCP have no private store handle or artifact tool
  -> Reader consumes only GET /build_intent/artifacts
      -> App refresh is asynchronous and request-sequenced,so overlay failure cannot gate reading
      -> RightRail renders four typed views and sends evidence LIDs through the existing goto path
      -> browser has no prepare/submit/fail endpoint wrapper and never receives goal,digest,task path,or executor detail
      -> comparison overflow is local to its table;desktop rail and mobile document retain their native scrolling
  -> integration boundaries remain fail closed on real books
      -> host classifies /build_intent and /build_workbench as API before SPA fallback
      -> Provider receives a bounded sampled catalog while candidate scope is checked against the complete LID catalog
      -> resident millisecond timestamps normalize to ISO only at the Sidecar dispatcher boundary
```
...
Intent replan and migration `[ADR-0093]`
  ...
  -> legacy compatibility is explicit
      -> only legacy-plan maps a one-command full build to confirmed standard_deep
