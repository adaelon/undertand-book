...
Interactive Workbench UI
  ...
  -> if readiness.route becomes reader, App re-runs reader initialization
```

```text
Source reconciliation review
  -> GET /book/build_workbench includes source_review report/unresolved/draft/decisions
  -> BuildWorkbenchPane shows Markdown/PDF/candidate evidence per unresolved block
  -> POST /build_workbench/source_review.resolve writes review-decisions.json
  -> job event records the review decision; deterministic gates still control reader handoff
```
