...
Codex plugin one-command prebuild
  ...
  -> buildAutomaticBuildSnapshot recomputes artifact/hash/schema status from disk
      ...
      -> automatic-build-executor-session.ts owns the public automatic_build_executor_session.v3 carrier;current public dispatches issue V3 records directly and legacy V1/V2 records remain read only
          ...
          -> generation.start.v3 accepts only the final batch ordinal,confirms the final batch,creates one body-free grant,and remains the sole transition into advance/claim/start
          ...
          -> the frozen input still passes the existing lease binding check,and create-only start replays the same semantic attempt after response loss
          ...
          -> build-executor-mcp.ts serves one thread-owned stdio connection whose non-serializable capability binds the first handoff and every derived session/input/grant/sink ref;parent and child do not share that connection
              ...
              -> timing v2 partitions successful generation.start into current-state/claim,input-render-or-reuse,persist/response and submit_candidate into candidate-gate,writer/commit,next-work-prepare;Core closes the first two boundaries and MCP closes the response tail with the same monotonic clock,while the strict reducer requires each phase sum to cover the server interval
