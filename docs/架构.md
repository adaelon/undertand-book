...
Runtime Agent request governance `[ADR-0091/0113]`
  ...
  -> agent_prompt.rs: base identity + evidence/source/discovery/navigation/Reader/paper/memory/profile/finish policies
      ...
      -> resident-agent.policy.source-delivery@v4 permits source.present only after observed source evidence and stops adjacent/range retries after SOURCE_NOT_OBSERVED
      -> resident-agent.policy.tool-discovery@v4 exposes only a bounded capability directory and evidence-topology rules;the model proposes semantic need fields but cannot claim Runtime authority
  ...
  -> orchestrator.rs: TurnLocatorLedger separately owns current-turn LID provenance and never upgrades a locator into source evidence
      ...
      -> structure/search/query/context project only their contract-declared LID fields;newly admitted TurnEvidenceLedger ranges add VerifiedEvidence origin
  ...
  -> orchestrator.rs: TurnProgressLedger owns the Runtime-derived monotonic ProgressPhase
      ...
      -> two consecutive unchanged model-tool batches arm ProgressPhaseGuard;the next registered batch fails before dispatch with AGENT_NO_PROGRESS even when arguments change
  ...
  -> orchestrator.rs: max_turns reserves one tools-disabled finalization sampling outside the model-tool loop budget
      ...
      -> a valid answer is delivered with TURN_LIMIT_EXCEEDED + incomplete honesty;native or textual tool invocation fails with FINALIZATION_TOOL_PROTOCOL_VIOLATION instead of re-entering dispatch
  ...
  -> auto_compaction.rs: each planned sampling computes active input + output reserve + safety margin
      ...
      -> loop-limit finalization performs the same physical-fit check with zero tool schemas and may compact before its sole reserved sample
...
Codex plugin one-command prebuild
  ...
  -> buildAutomaticBuildSnapshot recomputes artifact/hash/schema status from disk
      ...
      -> executor-transport.ts owns executor_transport_profile.v1 and the canonical response packer;the active BookStructure router measures every final chunk envelope against token,UTF-8 byte,envelope-reserve,and chunk-count caps
      ...
      -> automatic-build-executor-session.ts owns the internal automatic_build_executor_session.v2 carrier without changing the production V1 default
          -> executor.open.v2 revalidates the public dispatch and freezes exact prompt/input bytes behind one opaque generation_input_ref;the private input object is create-only while the delivery session and receipt ledger remain body-free
          -> DELIVER_INPUT returns only a two-segment manifest and next request;input.next emits final-envelope-measured chunks whose global ordinal,UTF-8 byte range,payload hash,and opaque receipt are session-bound
          -> replay of one next request is byte-identical;only a contiguous confirmed receipt prefix advances,and reopen projects the first unconfirmed chunk without creating task lease,execution.json,start.json,or semantic attempt
          -> the final receipt creates one body-free generation grant at semantic_attempt=0;grant replay and a lost control response do not mutate task state
          -> generation.start is the sole V2 transition into advance/claim/start;it re-renders the production input,compares frozen byte/hash identity,and persists one body-free GENERATE response for same-attempt replay
          -> accepted start create-only issues one candidate_sink_ref bound to the delivery session,generation grant,task identity,lease epoch,semantic attempt,output contract,and transport profile
          -> executor.submit_candidate.v2 accepts only a closed JsonValue;Core rejects host objects,aliases/cycles,non-finite numbers,depth/node/token/byte overflow,then serializes canonical UTF-8 once into the current attempt's create-only private mailbox
          -> same canonical hash replay records a submit revision without another writer;different hash conflicts fail closed;Schema,evidence,writer,quality,receipt,and artifact publication remain the existing downstream chain
          -> submit responses reopen only the candidate-free durable V2 action;candidate bodies,paths,and schema source values never enter tool results,root projection,or general logs
          -> automatic_build_failure_diagnostic.v3 separates input_delivery,generation,candidate_sink,and artifact_writer;unknown executor reports retain only an allowlisted executor_failed plus reported-code digest
          ...
          -> build-executor-tool-contract.ts owns the cycle-free server/tool identity;build-executor-tool-adapter.ts owns the closed agent-only schema;build-executor-mcp.ts serves it over one stdio process whose non-serializable capability binds the first handoff and every derived session/input/grant/sink ref
          -> the MCP adapter rejects time,path,capability,and unknown request fields before authorization;the server remeasures every result against executor_transport_profile.v1 and maps failures to one bounded bootstrap diagnostic
          -> T6 source publication switches the canonical custom-agent,executor skill,root build skill,doctor and source-only release gate to V2 while root/project MCP configuration remains negative
          -> T7 closes compiled Sidecar,thin-plugin installed parity,cachebuster,root-negative and real custom-agent CLI trace gates;only an explicitly authorized T8 guarded real-book retry may follow
  ...
  -> nextAutomaticBuildAction selects only the first unfinished stage
      -> extract: run deterministic input command
          ...
          -> the agent-only stdio connection opens V2,delivers bounded ordered chunks,accepts one generation grant,and creates semantic attempt 1 only at generation.start
          -> the dedicated child produces one strict JsonValue and submits it through executor.submit_candidate;no candidate source,path command,or shell fallback exists on V2
          -> T1 proves the old 317,247-byte V1 GENERATE shape incompatible;T2-T5 provide the bounded carrier,sink,BookStructure routing and phase-aware accounting;T6 publishes the source agent/plugin/doctor boundary
          ...
          -> V1 sessions remain read-only compatibility state;new V2 dispatch never falls back to V1 or rewrites the old policy generation
...
Windows desktop distribution
  ...
  -> $understand-book-build invokes the installed Bun sidecar
      -> published root skill retains semantic goal/Blueprint planning and exact user confirmation,then drives only the four build.step actions
          -> SPAWN_EXECUTORS fills live_by_ref up to live capacity;the first owned child terminal triggers an immediate build.step reread while every other live ref remains owned;WAIT honors retry_after_ms only without a live child;NEEDS_USER and DONE require no orphaned refs
      ...
      -> build.step routes bounded create/step stdin envelopes through automatic-build-driver.ts `[ADR-0101]`
          ...
          -> executor.open consumes one closed tool request plus the process-private child connection capability
              ...
              -> terminal receipt projects DONE;active state atomically creates/reuses one body-free delivery session without task attempt
              -> input.next advances only a contiguous receipt prefix;the final receipt issues a grant at attempt 0;generation.start atomically creates or replays the same semantic attempt
              -> submit_candidate validates one closed JsonValue into the canonical mailbox,invokes the existing writer,and advances only from canonical task/dispatch receipts
