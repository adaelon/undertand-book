...
Codex plugin one-command prebuild
  ...
  -> buildAutomaticBuildSnapshot recomputes artifact/hash/schema status from disk
      ...
      -> automatic-build-executor-session.ts owns the public automatic_build_executor_session.v3 carrier;current public dispatches issue V3 records directly and legacy V1/V2 records remain read only
          ...
          -> DELIVER_INPUT returns a segment manifest and next request;input.next advances by previous_chunk_ordinal and emits chunks bound by global ordinal,contiguous UTF-8 byte range,and direct payload length
          -> replay of one next request is byte-identical;reopen projects the first unconfirmed ordinal without creating task lease,execution.json,start.json,or semantic attempt
          -> final delivery creates one body-free generation grant at semantic_attempt=0 with final_delivered_ordinal and explicit output_schema_version
          -> generation.start is the sole V3 transition into advance/claim/start;it re-renders the production input and compares the assembled prompt/input bytes directly
          ...
          -> executor.submit_candidate accepts only a closed JsonValue;Core rejects host objects,aliases/cycles,non-finite numbers,depth/node/token/byte overflow,then serializes canonical UTF-8 once into the current attempt's create-only private mailbox
          ...
          -> submit responses reopen only the candidate-free durable V3 action;candidate bodies,paths,and schema source values never enter tool results,root projection,or general logs
  ...
  -> nextAutomaticBuildAction selects only the first unfinished stage
      -> extract: run deterministic input command
          ...
          -> no V1/V2 handoff is superseded or rewritten;unfinished work enters through a newly confirmed current plan and a directly issued V3 handoff
