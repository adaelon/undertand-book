...
协议：quantification-essence-agent-v1；模型：`deepseek-v4-flash`；开始：2026-09-08T05:35:35.228Z；状态：completed。
...
Token 来自全部实际 Provider 请求，包含工具循环、内层查询、压缩和答案修复；不使用 Runtime 估算。问答 Agent 共 0 次请求，Chunk 共 0 次。缺失 usage 则均值为 N/A。判分另计：4 次请求、3,087 Token，不计入被测系统成本。P95 排除服务启动、预构建和事后判分，包含失败任务。

全流程（不含判分）实际发起 26 次模型请求。后台画像 review 包含在用量中，单列 0 次、0 次缺失 usage；缺失项没有用估计值补齐。
