...
协议：quantification-essence-agent-v1；模型：`deepseek-v4-flash`；开始：2026-09-08T05:42:21.241Z；状态：completed。

## 共同问答

| 系统 | 证据召回 | 引用支持率¹ | 任务成功率¹ | P95 延迟 | 单任务 Token |
| --- | ---: | ---: | ---: | ---: | ---: |
| Chunk RAG (BM25, single-shot) | 100.0% | 50.0% | 50.0% (3/6) | 4.96 s | 4,350 |
| Resident Agent (LID + Graph, full loop) | 50.0% | 25.0% | 33.3% (2/6) | 300.01 s | 79,006 |
...
Token 来自全部实际 Provider 请求，包含工具循环、内层查询、压缩和答案修复；不使用 Runtime 估算。问答 Agent 共 39 次请求，Chunk 共 6 次。缺失 usage 则均值为 N/A。判分另计：11 次请求、21,149 Token，不计入被测系统成本。P95 排除服务启动、预构建和事后判分，包含失败任务。

全流程（不含判分）实际发起 45 次模型请求。后台画像 review 包含在用量中，单列 0 次、0 次缺失 usage；缺失项没有用估计值补齐。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| exact-04 | Chunk | ✓ | ✓ | 2 | 1 | 4,235 |
| exact-04 | Agent | ✓ | ✓ | 2 | 4 | 31,786 |
| concept-03 | Agent | ✓ | ✗ | 0 | 6 | 49,084 |
| concept-03 | Chunk | ✓ | ✓ | 1 | 1 | 4,217 |
| cross-02 | Chunk | ✓ | ✗ | 0 | 1 | 4,456 |
| cross-02 | Agent | ✗ | ✗ | 0 | 6 | 95,758 |
| cross-04 | Agent | ✗ | ✗ | 0 | 8 | 106,495 |
| cross-04 | Chunk | ✗ | ✗ | 4 | 1 | 4,276 |
| formula-01 | Chunk | ✓ | ✓ | 3 | 1 | 4,435 |
| formula-01 | Agent | ✓ | ✓ | 1 | 7 | 75,370 |
| formula-03 | Agent | ✓ | ✗ | 0 | 8 | 115,544 |
| formula-03 | Chunk | ✗ | ✗ | 1 | 1 | 4,478 |
