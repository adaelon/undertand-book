...
协议：quantification-essence-agent-v1；模型：`deepseek-v4-flash`；开始：2026-09-08T05:45:52.916Z；状态：completed。

## 共同问答

| 系统 | 证据召回 | 引用支持率¹ | 任务成功率¹ | P95 延迟 | 单任务 Token |
| --- | ---: | ---: | ---: | ---: | ---: |
| Chunk RAG (BM25, single-shot) | 100.0% | 100.0% | 100.0% (1/1) | 3.98 s | 4,062 |
| Resident Agent (LID + Graph, full loop) | 100.0% | 100.0% | 100.0% (1/1) | 46.50 s | 85,504 |
...
Agent 问答内容正确率：100.0%；Chunk：100.0%。内容正确不代表引用或任务完成。

Token 来自全部实际 Provider 请求，包含工具循环、内层查询、压缩和答案修复；不使用 Runtime 估算。问答 Agent 共 7 次请求，Chunk 共 1 次。缺失 usage 则均值为 N/A。判分另计：2 次请求、3,706 Token，不计入被测系统成本。P95 排除服务启动、预构建和事后判分，包含失败任务。

全流程（不含判分）实际发起 8 次模型请求。后台画像 review 包含在用量中，单列 0 次、0 次缺失 usage；缺失项没有用估计值补齐。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| concept-02 | Chunk | ✓ | ✓ | 2 | 1 | 4,062 |
| concept-02 | Agent | ✓ | ✓ | 1 | 7 | 85,504 |
