...
协议：quantification-essence-agent-v1；模型：`deepseek-v4-flash`；开始：2026-09-08T05:50:53.576Z；状态：completed。

## 共同问答

| 系统 | 证据召回 | 引用支持率¹ | 任务成功率¹ | P95 延迟 | 单任务 Token |
| --- | ---: | ---: | ---: | ---: | ---: |
| Chunk RAG (BM25, single-shot) | N/A | N/A | 75.0% (3/4) | 3.81 s | 3,906 |
| Resident Agent (LID + Graph, full loop) | N/A | N/A | 75.0% (3/4) | 53.61 s | 59,114 |
...
Agent 问答内容正确率：75.0%；Chunk：75.0%。内容正确不代表引用或任务完成。

Token 来自全部实际 Provider 请求，包含工具循环、内层查询、压缩和答案修复；不使用 Runtime 估算。问答 Agent 共 28 次请求，Chunk 共 4 次。缺失 usage 则均值为 N/A。判分另计：8 次请求、7,106 Token，不计入被测系统成本。P95 排除服务启动、预构建和事后判分，包含失败任务。

全流程（不含判分）实际发起 63 次模型请求。后台画像 review 包含在用量中，单列 3 次、0 次缺失 usage；缺失项没有用估计值补齐。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| refusal-01 | Chunk | ✓ | ✓ | 0 | 1 | 3,857 |
| refusal-01 | Agent | ✗ | ✗ | 0 | 7 | 55,577 |
| refusal-02 | Agent | ✓ | ✓ | 0 | 5 | 26,880 |
| refusal-02 | Chunk | ✗ | ✗ | 0 | 1 | 4,231 |
| refusal-03 | Chunk | ✓ | ✓ | 1 | 1 | 3,810 |
| refusal-03 | Agent | ✓ | ✓ | 0 | 8 | 80,312 |
| refusal-04 | Agent | ✓ | ✓ | 0 | 8 | 73,685 |
| refusal-04 | Chunk | ✓ | ✓ | 0 | 1 | 3,725 |
