...
if (!book || idStr === undefined) {
  console.error(`usage: tsx emit-input.ts <book.md|epub> <windowId> [--book-id <id>] ${contentProfileUsage()}`);
  process.exit(2);
}
...
process.stdout.write(buildProfiledPass1Input(w, byLid, source, parsedProfile.contentProfile).text);
